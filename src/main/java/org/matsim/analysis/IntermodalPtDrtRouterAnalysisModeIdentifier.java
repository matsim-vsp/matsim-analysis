 ...
 *
 */
public final class IntermodalPtDrtRouterAnalysisModeIdentifier implements AnalysisMainModeIdentifier {
	private final List<String> modeHierarchy = new ArrayList<>() ;
	private final List<String> drtModes;
	private static final Logger log = LogManager.getLogger(IntermodalPtDrtRouterAnalysisModeIdentifier.class);
	public static final String ANALYSIS_MAIN_MODE_PT_WITH_DRT_USED_FOR_ACCESS_OR_EGRESS = "pt_w_drt_used";

	@Inject
	public IntermodalPtDrtRouterAnalysisModeIdentifier() {
	...
	@Override public String identifyMainMode( List<? extends PlanElement> planElements ) {
		...
		if (mainMode.equals(TransportMode.pt)) {
			...
			if (isDrtPt) {
				return IntermodalPtDrtRouterAnalysisModeIdentifier.ANALYSIS_MAIN_MODE_PT_WITH_DRT_USED_FOR_ACCESS_OR_EGRESS;
			} else {
